...
matchesRoute.get('/matches/accepted/:userId', async (req, res) => {
    const { userId } = req.params
    try {
        const matches = await Match.find({ $or: [{ user1: userId }, { user2: userId }], user1_status: 'accepted', user2_status: 'accepted' })
        return res.json(matches)
